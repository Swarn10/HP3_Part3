...
def plotGPUData(mem_file, colors, plot_dir):
    ...
    plt.title("GPU Memory Used for one RCNN Run [Batchsize = 1]")
...
if __name__ == "__main__":
    ...
    if not os.path.exists("./memory_direct_shared.txt"):
        print ("Direct Convolution log not found..Skipping")
    else:
        MEM_FILE['DIRECT_SHARED'] = "./memory_direct_shared.txt"
    ...
    if not os.path.exists("./memory_im2col_cublas.txt"):
        print ("Im2Col Convolution log not found..Skipping")
    else:
        MEM_FILE['IM2COL_CUBLAS'] = "./memory_im2col_cublas.txt"
